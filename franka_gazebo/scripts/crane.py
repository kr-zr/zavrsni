...
class Kretanje():
    ...
    def left_sensor_callback(self,data):
        ...
        if self.kreni == False and self.brojac > 0:                      #ovo radimo na pocetku kad biramo nasumicni x,y u prostoru u nekom range-u
            self.pose_goal.position.x = random.uniform(0.3, 0.5)
            self.pose_goal.position.y = random.uniform(-0.2, 0.2)
            self.kreni = True
        if not self.sensor_data_left.states and self.pose_goal.position.z > 0.45 and self.brojac > 0:     #ako nismo ostvarili dodir ili nismo se jos dovoljno spustili, nastavi se spustati
            #print("2")
            self.pose_goal.position.z = self.pose_goal.position.z - 0.04
            self.move_arm.set_pose_target(self.pose_goal)
            self.plan = self.move_arm.go()
            self.move_arm.stop()
        if (self.sensor_data_left.states or self.pose_goal.position.z <= 0.45) and self.brojac > 0:   #ako smo se dovoljno spustili i nismo nista nasli ili smo nasli tocku
            #print("3")
